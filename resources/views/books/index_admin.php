<form class="navbar-form " action="<?= \core\router\generate('admin1') ?>">
    <label>
...
<form class="navbar-form navbar" action="<?= \core\router\generate('admin1') ?>">
...
<?php if (!empty($books)) foreach ($books as $book): ?>
    <div class="media">
        <div class="media-left">
            <a href="<?= \core\router\generate('book_by_id_admin', ['id' => $book['id']]) ?>">
                <img src="<?= $book['poster'] ?>" alt="<?= $book['name'] ?>" class="media-object" width="300">
            </a>
        </div>
        <div class="media-body">
            <h4 class="media-heading">
                <a href="<?= \core\router\generate('book_by_id_admin', ['id' => $book['id']]) ?>"><?= $book['name'] ?></a>
            </h4>
            ...
            <a href="<?= \core\router\generate('book_by_id_admin', ['id' => $book['id']]) ?>" class="btn btn-primary">Details</a>
            <form  action="<?= \core\router\generate('admin1');?>">
                <button class="btn btn-primary" name="delete" value="<?= $book['id']?>"> Delete </button>
            </form>
        </div>
    </div>
<?php endforeach; ?>
    ...
    <ul class="pagination">
        <?php for ($i = 0; $i < $pages; $i++): ?>
            <li class="<?= $i == $current ? 'active' : '' ?>">
                ...
                $page = !empty($params) ? \core\router\generate('admin1') . '?' . http_build_query($params) : \core\router\generate('admin1');
                ?>
                <a href="<?= $page ?>">
                    <?= ($i + 1) ?>
                </a>
